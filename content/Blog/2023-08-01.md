Title: Mounting Synology NFSv4 shares in Linux using Kerberos authentication
Status: published
...
- **Configure Kerberos database**  
    - After initial creation of the database, use `$sudo kadmin.local` to add the administrator principal to the database. After having added this principal, you can from now on use `$kadmin` which interacts with the database _via the kadmind daemon_.
